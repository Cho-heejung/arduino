...
// adjust servo position according to the USS read value
  ...
  // add your code here!
  float _DUTY_N = 0;
  if ((float)(dist_raw / 10) <= 18) {
    myservo.writeMicroseconds(_DUTY_MIN);
    if ((float)(dist_raw / 10) == 18) digitalWrite(PIN_LED, 0);
    else digitalWrite(PIN_LED, 1);
  }
  else if ((float)(dist_raw / 10) >= 36) {
    myservo.writeMicroseconds(_DUTY_MAX);
    if ((float)(dist_raw / 10) == 36) digitalWrite(PIN_LED, 0);
    else digitalWrite(PIN_LED, 1);
  }
  else {
    _DUTY_N = 553 + (float)((dist_raw - 180) / 10) * 102.6;
    myservo.writeMicroseconds(_DUTY_N);
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min) reading = reading; 
  else if(reading > dist_max) reading = reading;
